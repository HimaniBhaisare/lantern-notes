...
async function mdToNote(mdData) {
  ...
  lines.forEach((line, index) => {
    ...
    if (
      (blockType == 'list' && prevBlockType == 'list') ||
      (blockType == 'checklist' && prevBlockType == 'checklist') ||
      (blockType != 'code' && isCodeBlock)
    ) {
...
function parseMdNestedList(line, itemsObject, numberOfTabs) {
  ...
  return parseMdNestedList(
    line,
    itemsObject[itemsObject.length - 1].items,
    --numberOfTabs,
  );
}
