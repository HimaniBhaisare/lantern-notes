  ...
  if (password != cnfPassword) {
    ...
    auth
      ...
      .then((userCredential) => {
        ...
        storeUserDetails(user, name).catch((err) => console.log(err.message));
      })
      .catch((error) => {
        let errorCode = error.code;
        if (errorCode == 'auth/email-already-in-use') {
          ...
          signupError.textContent =
            '*Password is too weak. Use min. 6 characters!';
        } else {
  ...
  if (firebaseUser) {
    if (firebaseUser.displayName) {
      ...
      document
        ...
        .setAttribute('src', `assets/profilePictures/userAvatar.png`);
      document.getElementById('loggedinMessageSpan').textContent =
    ...
    if (
      currentNote.mdContent != defaultNote.mdContent ||
      currentNote.blockContent != defaultNote.blockContent ||
      currentNote.noteName != defaultNote.noteName
    ) {
